...
fig4_ready <- fig4_raw %>%
  ...
  mutate(metric = case_when(metric == 'underemployment' ~ '% under-employed',
                            ...
                            metric == 'regulated' ~ '% in regulated occupation',
                            .default = NA),
         group = case_when(group == 'diff_man' ~ 'Men: migrants - native',
                           group == 'diff_woman' ~ 'Women: migrants - native',
                           group == 'diff_woman_man' ~ 'Difference in gap between men and women',
                           .default = group)) %>%
  pivot_wider(names_from = 'group', values_from = 'vals') %>%
  mutate(`Men: migrants - native` = case_when(metric != 'mean income decile' ~ as.numeric(`Men: migrants - native`) * 100,
                                              .default = as.numeric(`Men: migrants - native`)),
         `Women: migrants - native` = case_when(metric != 'mean income decile' ~ as.numeric(`Women: migrants - native`) *100,
                                              .default = as.numeric(`Women: migrants - native`)),
...
fig5_ready <- fig5_raw %>%
  ...
  rename('% under-employed difference' = 'uemp_diff',
         ...
         '% over-qualificatied difference' = 'overed_diff', 
...
output_fp_map <- paste(output_fp, 'maps/')
dir.create(output_fp_map, showWarnings = F)

st_write(fig5_ready, paste0(output_fp_map, 'fig5_bw_regional.geojson'))

### Fig 6 ###
fig6_raw <- read.csv(paste0(cur_date, 'Results/scratchpad/rq03_01/hatfield_overed_underemp_diff.csv'))

fig6_ready <- fig6_raw_all %>%
  dplyr::select(-X) %>%
...
fig9_ready <- fig9_raw %>%
  ...
  mutate(group = case_when(names %in% c('basic_wage_diff_mean', 'educ_wage_diff_mean', 'uemp_penalty', 'overed_penalty') ~ 'earnings differences (EURO)',
                           names %in% c('macro_potential_basic', 'macro_potential_uemp', 'macro_potential_educ', 'macro_potential_overed') ~ 'lost wages due to Brainwaste (EURO)',
         ...
         model = case_when(grepl('basic', names) ~ 'basic wage model',
                           ...
                           grepl('uemp', names) ~ 'scenario: % immigrant under-employed = % native under-employed',
                           grepl('overed', names) ~ 'scenario: % immigrant over-qualified = % native over-qualified',
                           .default = NA)) %>%
...
### Fig 10 ###
fig10_raw <- read.csv(paste0(cur_date, 'Results/h3_1/education_brainwaste_shortage.csv'))

fig10_ready <- fig10_raw %>%
...
write.csv(fig10_ready, paste0(output_fp, 'fig10_shortage.csv'))

### Fig 11 ###
fig_11_descr_raw <- read.csv(paste0(cur_date, 'Results/h4/educated_abroad_desc.csv'))
fig_11_regr_raw <- read.csv(paste0(cur_date, 'Results/h4/educated_abroad_regr.csv'))
...
write.csv(fig_11_ready, paste0(output_fp, 'fig11_educated_abroad.csv'))  
  
### Fig 12 ###
fig_12_descr_raw <- read.csv(paste0(cur_date, 'Results/h4/host_lang_desc.csv'))
fig_12_regr_raw <- read.csv(paste0(cur_date, 'Results/h4/host_lang_regr.csv'))
...
fig_12_ready <- fig_12_descr_raw %>%
  dplyr::select(!X) %>%
  filter(!is.na(host_lang_factor), n > 50) %>%
  pivot_longer(cols = ends_with('mean'), names_to = 'metric', values_to = 'descriptive_mean') %>%
  mutate(metric = str_replace(metric, '_mean', '')) %>%
  left_join(fig_12_regr, by = c('host_lang_factor' = 'term', 'metric' = 'dv', 'COUNTRY')) %>%
  mutate(significance = case_when(p.value < 0.05 & (sign(descriptive_mean) == sign(estimate)) ~ 'difference from natives statistically significant',
                                  p.value < 0.05 & (sign(descriptive_mean) != sign(estimate)) ~ 'difference from natives statistically significant, but sign changed',
                                  p.value >= 0.05 ~ 'difference from natives not statistically significant',
                                  .default = NA),
...
write.csv(fig_12_ready, paste0(output_fp, 'fig12_host_lang.csv'))  


### Fig 13 ###
fig_13_descr_raw <- read.csv(paste0(cur_date, 'Results/h4/recognition_desc.csv'))
fig_13_regr_raw <- read.csv(paste0(cur_date, 'Results/h4/recognition_regr.csv'))
...
fig_13_ready <- fig_13_descr_raw %>%
  dplyr::select(!X) %>%
  filter(!is.na(received_recognition_factor), n > 50) %>%
  pivot_longer(cols = ends_with('mean'), names_to = 'metric', values_to = 'descriptive_mean') %>%
  mutate(metric = str_replace(metric, '_mean', '')) %>%
  left_join(fig_13_regr, by = c('received_recognition_factor' = 'term', 'metric' = 'dv', 'COUNTRY')) %>%
  mutate(significance = case_when(p.value < 0.05 & (sign(descriptive_mean) == sign(estimate)) ~ 'difference from natives statistically significant',
                                  p.value < 0.05 & (sign(descriptive_mean) != sign(estimate)) ~ 'difference from natives statistically significant, but sign changed',
                                  p.value >= 0.05 ~ 'difference from natives not statistically significant',
                                  .default = NA),
...
write.csv(fig_13_ready, paste0(output_fp, 'fig13_recognition.csv'))  


### Fig 14 ###
fig_14_descr_raw <- read.csv(paste0(cur_date, 'Results/h4/residence_desc.csv'))
fig_14_regr_raw <- read.csv(paste0(cur_date, 'Results/h4/residence_regr.csv'))
...
fig_14_ready <- fig_14_descr_raw %>%
  dplyr::select(!X) %>%
  filter(!is.na(yearesid_10_years_factor), n > 50) %>%
  pivot_longer(cols = ends_with('mean'), names_to = 'metric', values_to = 'descriptive_mean') %>%
  mutate(metric = str_replace(metric, '_mean', '')) %>%
  left_join(fig_14_regr, by = c('yearesid_10_years_factor' = 'term', 'metric' = 'dv', 'COUNTRY')) %>%
  mutate(significance = case_when(p.value < 0.05 & (sign(descriptive_mean) == sign(estimate)) ~ 'difference from natives statistically significant',
                                  p.value < 0.05 & (sign(descriptive_mean) != sign(estimate)) ~ 'difference from natives statistically significant, but sign changed',
                                  p.value >= 0.05 ~ 'difference from natives not statistically significant',
                                  .default = NA),
...
write.csv(fig_14_ready, paste0(output_fp, 'fig14_residence.csv'))  
  ...
  for(i in 1:nrow(treat_vars)){
    ...
    for(cur_comp in comp_groups){
      ...
      for(dv in dependent_vars){
          ...
          fit_fp <- paste0(paste(input_fp, country, treat_var, cur_comp, sep = '/'), '/', dv, '_', as.character(treat_year), '_control.csv')
          #load model file
...
###Fig 1###
fig16_raw <- read.csv(paste0(cur_date, 'Results/scratchpad/sumtables/hypothesis_table_immigrants.csv'))
...
write.csv(fig16_ready, paste0(output_fp, 'fig1_htable_college.csv'))
